...
# -pthread: the solver has a thread of its own, see simthread.cpp, so that the
# simulation rate is not tied to the screen's refresh rate.
CC := g++ -DPCCTS_USE_NAMESPACE_STD -fopenmp -pthread
...
main.o: fluid.h particle.h vektor.h autogenerated.h shaders.h simthread.h scenes.h
scenes.o: fluid.h particle.h vektor.h scenes.h
shaders.o: shaders.h
simthread.o: fluid.h particle.h vektor.h autogenerated.h simthread.h
...
# diagnostic probe: run a scene without a window and count what stops the
# particles in it. Not part of check - it reports, it does not judge.
test/probe: test/probe.cpp scenes.cpp fluid.h particle.h vektor.h scenes.h
	$(CC) -DFLUID_PROBE -O2 -g -o $@ test/probe.cpp scenes.cpp
